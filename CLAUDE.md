...
## Source of truth
- **Rules:** `docs/SPEC.md` (Regolamento v1.1). Never invent a rule. If a rule is missing or ambiguous, stop and ask the owner.
- **Acceptance tests `AT-xx`** in `docs/SPEC.md` are the contract. Every rule has an `AT-xx` test with the same ID in its name (e.g. `AT_05_EqualValueForbidsSum`).
...
- After the first play of a round, no two table cards share the same value.
